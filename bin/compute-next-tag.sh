...
# published. It is at v1.0.0-0.
#
# This role is unusual among the roles of this fleet in that it deploys no
# software: it is a host-level housekeeping role with no container image, no
# upstream project and therefore no version of its own. `defaults/main.yml`
# holds nothing a version could be read out of, and Renovate has no leaf here to
# bump. The `1.0.0` in the tags is a number somebody picked by hand.
#
...
# bugfix, a feature, a dependency bump - release itself without a human tagging
# it. The workflow this replaced looked for a `renovate[bot]` commit whose
# subject mentioned "docker tag to"; because there is no Docker tag here for
# Renovate to bump, it had never once produced a tag.

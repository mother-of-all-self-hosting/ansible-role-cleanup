...
# numbers and duration strings that a script looking for something version-shaped
# could latch onto. `system_cleanup_kernels_keep: 1` would become `v1-0` and
# `system_cleanup_logs_retention: 7d` would become `v7d-0`; neither is a version
...
scenario() {
	...
	YAML
	printf 'placeholder\n' > files/purge-old-kernels
	printf 'placeholder\n' > meta/main.yml
	printf 'placeholder\n' > tasks/main.yml
	printf 'placeholder\n' > templates/systemd-journald-autovacuum.service.j2
...
edit_defaults="printf 'system_cleanup_extra: false\n' >> defaults/main.yml"
...
# A script grepping for something version-shaped would answer v1-0 or v7d-0 here.
